...
if (switches.outputs)	options.idx.push(switches.outputs);
...
receiver.on('power', function(pwr) {
	...
	if (!pwr) {
		...
		if (switches.outputs)	domoticz.switch(switches.outputs,0);
		...
		if (switches.outputs)	domoticz.switch(switches.outputs,255);
...
// receiver: listening output
receiver.on('listenOutput', function(output,outputName) {
	if (switches.outputText)		domoticz.device(switches.outputText,0,outputName);
});
